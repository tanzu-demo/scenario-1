...
```
tanzu apps workload create customer-profile-ui -f config/workload.yaml \
  --local-path . \
  --source-image dev.local/customer-profile-ui-source \
  --type web
```
...
```
tanzu apps workload get customer-profile-ui
```
